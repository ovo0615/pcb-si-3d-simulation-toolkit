...
兩份驗證都刻意包含**負控制組**，呈現方法失效時的樣子。分段驗證中發現「切面處參考平面未縫合」會產生 **14 dB** 的假諧振，這個條件已直接成為工具的自動檢查並擋在使用者前面。驗證的價值不在背書，而在找出方法的邊界，再把邊界寫回工具裡。

通道裁切與 Layout 清理的等效性尚未單獨量化驗證，列為後續工作，此處不作宣稱。
