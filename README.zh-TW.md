...
現有的混合求解流程（如 HFSS Regions in SIwave）要工程師自己框出 3D 區域，軟體才接手。本工具自動搜尋安全切面、逐段評估 3D 複雜度、自動決定哪一段給 HFSS、哪一段給 SIwave；工程師的工作只剩下選段數，以及覆寫不同意的那幾段。

**模型有錯不擋路：自動修復、留下審計鏈、照樣開跑。**
業界的 IBIS 檢查器（ibischk 與各家工具內建的）只驗不修，驗出錯就把人擋在門外。本工具把有問題的模型修成一份受管副本，記下修了什麼、蓋上 SHA-256 指紋，然後照樣開跑；簽核時每一份模型的來歷都說得清楚。

**TDR 看到劇變，Layout 上直接標給你——然後在那個位置虛擬剖一刀。**
業界的做法是拿 TDR 的距離數字回去對圖、再剖板確認。本工具把求解或量測來的 S 參數轉成 TDR，依 |dZ/dx| 峰值排序劇變、換算成走線路徑座標直接標在 Layout 上，劇變表逐列可一鍵在該處取 Q2D 截面；[定位驗證](./validation/tdr/TDR_群延遲定位驗證報告.md)平均誤差 0.66 mm、最大 1.71 mm，截面從 EDB 重建，板子不用剖。

**等化掃描單組 3 秒，完整模擬同一組合要 48～212 秒——Top-3 排名與完整模擬完全一致。**
參數掃描不是新東西，秒級加上有證據的可信度才是。別人每一格都是完整模擬，本工具用統計眼引擎把 Tx×Rx 全組合秒級排完；排名一致性（抖動輕的組合 Spearman ρ＝0.86～0.90、Top-3 集合全樣本一致）、差距的來源（抖動 budget）與使用邊界，都量化在 [26 組矩陣實測報告](./validation/ibis/秒級眼圖預測與完整模擬對照_20260829.md)裡。既定用法也寫在報告裡：掃描挑方向、完整模擬簽核。

**按一顆按鈕，證明這顆模型解得動——不是只過語法檢查。**
語法全過的模型照樣可能解不動。本工具的健檢拿真實參考通道把模型實際解一次，配不起來的 Tx／Rx 組合在選單上當場停用；「跑到一半才發現模型有問題」這件事，消滅在按下求解之前。

前三項的「查無同類」依 2026 年 8 月對公開市售產品文件與文獻的查證。查證不能證明全世界沒有，只能證明我們認真找過。
...
<details>
<summary><b>微帶線的根因不是縫合——我們推翻了自己前一天的結論</b>（展開看過程）</summary>

專用試片把縫合密度從半徑 3 mm 內 14 顆掃到 0 顆，偏差散布只有 **0.001 dB**；代價來自切開本身，隨刀數累加。**補接地 Via 沒有用，該做的是少切幾刀。** 過程留在報告裡：[微帶線縫合密度驗證](./validation/stitch_density_microstrip/微帶線縫合密度驗證報告.md)｜[真實板全頻對照](./validation/segmentation/真實板全頻對照_20260817.md)
...
第一次用請走 [第一次跑](./docs/manual/00-第一次跑.md)——從下載到第一張眼圖，一條路、30 分鐘。
完整參考見 [操作說明](./操作說明.md)（十一章，含裁切、Port、分段、排程、IBIS 眼圖、TDR、截面阻抗與疑難排解）。
