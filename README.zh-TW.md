...
**等化掃描單組 3 秒，完整模擬同一組合要 48～212 秒——排名一致性 Spearman ρ＝0.90。**
參數掃描不是新東西，秒級加上有證據的可信度才是。別人每一格都是完整模擬，本工具用統計眼引擎把 Tx×Rx 全組合秒級排完；排名與 AEDT 完整模擬的一致性、差距的來源（抖動 budget）與偏差的方向，都量化在 [26 組矩陣實測報告](./validation/ibis/秒級眼圖預測與完整模擬對照_20260829.md)裡。既定用法也寫在報告裡：掃描挑方向、完整模擬簽核。
