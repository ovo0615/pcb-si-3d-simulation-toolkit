...
驗證不只是「證明現在夠好」，也用來**找出自己哪裡不夠好**。2026 年 8 月的
四項研究就是為了檢驗工具內部兩個從未被量化驗證的決策——切面品質怎麼判、
求解器怎麼指派——結果找出了三個真實的問題，並且都已修正：

| 發現 | 已做的修改 |
|---|---|
| HFSS 自適應預設值在掃頻上限**少算 3.2 dB 損耗** | 自適應範圍改為涵蓋整個掃頻範圍 |
| 切面品質檢查會放行**串接誤差 21.9 dB** 的切面 | 改用「縫合間距 → 可信頻率上限」判定，並經雙求解器與五種疊構驗證 |
| 走線彎折被誤當成複雜度，把 SIwave 完全能處理的段落派給 HFSS（慢 120 倍） | 彎折不再提高複雜度分數 |

[四項研究總覽](./validation/分段與求解器研究總覽.md)｜[English overview](./validation/solver_and_segmentation_studies.md)

原始數據、失敗的試片版本、以及推翻自己先前結論的過程全部公開。
