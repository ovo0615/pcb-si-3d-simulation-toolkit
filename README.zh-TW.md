...
| **IBIS 模型與眼圖** | 綁錯一側的眼圖照樣畫得出來 | 受管副本＋逐檔 SHA-256 信任；8 埠以上的完整通道用**一顆** Touchstone 元件建電路，保留全部耦合；兩側各一份 `.ibs`，綁定依可稽核的鍵吻合分三級，證據不足就要求人工指定 |
| **DDR 時序裕度** | 量到的時間**不是**裕度 | 裕度 ＝ 量到的時間 − 規格要求 − derating，而且逐個選通邊各自查表後取最小；沒有規格設定檔就只出時間、不判 Pass／Fail，工具不內建任何 JEDEC 數值 |
| **多埠通道串擾** | 隨機碼型要跑多久才碰得到最差對齊？ | 決定性最差碼型 **128 UI／3.1 s**，對照隨機碼型 20,000 UI／116.6 s；前者每條道**必然**發生一次，後者只是期望 1.22 次。組內有選通的叫位元組通道（量時序裕度）、沒有選通的是多道串擾（量眼圖劣化），**兩者的數值不可互相比較** |
...
**那個條件不是小字，是主詞。** 真實板上的微帶線通道切兩刀時，分段串接到 1 GHz 仍然只差 0.040 dB，但**可用頻率上限只剩 1.35 GHz**，3 GHz 以上就長到 1.5 dB 以上。

**微帶線的根因不是縫合。** 專用試片把縫合密度從半徑 3 mm 內 14 顆掃到 0 顆，偏差散布只有 **0.001 dB**；代價來自切開本身，隨刀數累加。**補接地 Via 沒有用，該做的是少切幾刀。** 這一條推翻了前一天自己的歸因，過程留在報告裡。[微帶線縫合密度驗證](./validation/stitch_density_microstrip/微帶線縫合密度驗證報告.md)｜[真實板全頻對照](./validation/segmentation/真實板全頻對照_20260817.md)
