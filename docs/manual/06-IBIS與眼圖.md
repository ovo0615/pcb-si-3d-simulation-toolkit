...
> **需要先有**：兩側的 `.ibs`，以及串接後的 Touchstone。
> （只想先看模型本身的話，「快速檢驗」不需要 Touchstone。）
> **讀完你會有**：眼圖與眼高眼寬、DDR 的時序裕度、或 SerDes 的等化建議。
> **這章很長**，因為它其實是四件不同的事——**用下面這張表跳到你要的那一段，
> 不要從頭讀。**
...
| 看不懂結果裡的數字 | [眼圖的那些數字](#眼圖的那些數字是怎麼算出來的) |
