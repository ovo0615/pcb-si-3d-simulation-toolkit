...
第 1 步除了「瀏覽…」之外，還有一個 **「帶入串接結果」**。
本次工作階段只要串接過（或用「外部 S 參數檔」載入過）一份通道，
這個按鈕就會出現，旁邊寫著檔名與埠數。

按下去就把路徑填好，不必去檔案總管翻。
**這也順便擋掉一種錯**：手動貼路徑很容易貼到上一次的那一份，
而舊檔照樣跑得起來，只是答案是上一次的。

「IBIS-AMI 通道」與「多埠通道」兩頁的第一步也有同一個按鈕。
...
**DDR 的接收端不是照一條固定的線判 0 和 1。** 那條線叫 Vref，規格允許它在一個
範圍內浮動，而規格要看的是**整個範圍內最差**的眼寬，不是某一個位置的眼寬。

只量一個位置的話，數字會偏樂觀。同一條通道實測：

| | Vref | 眼寬 |
|---|---|---|
| 只量求解器自動算的那一點 | 0.7577 V | 0.9087 UI |
| 掃規格 ±6% 取最差 | 0.6668 V | **0.8006 UI** |

**差 0.108 UI，那是原本以為有、其實沒有的餘裕。**

### 怎麼用

在右側「IBIS 模型與眼圖分析」→「標準 IBIS 通道」，走到**第 5 步〈分析設定〉**，
勾「掃判準位（Vref），取最差眼寬」，然後填三個數字：

![IBIS 通道精靈第 5 步：勾選掃判準位之後出現 VDDQ、範圍、步距三個欄位](../../graph/Vref掃描設定_20260818.png)

| 欄位 | 填什麼 |
|---|---|
| VDDQ（V） | 那條匯流排的電源電壓，照資料表填 |
| 範圍 ±（%） | 規格允許 Vref 偏多少，照資料表填 |
| 步距（%） | 每次移動多少 |

打字的時候下面就會顯示要掃幾點、範圍多寬。**點數上限是 81 點**，超過會自動把
步距放寬並把這件事寫出來，不會靜靜地少掃幾點。

### 為什麼幾乎不花時間

**判準位是後處理，不是求解設定。** 換一個 Vref 只是換一條水平線去量同一張眼圖，
求解結果整份重用。實測**每點 0.14 秒**，掃 13 點 1.8 秒。

所以這個選項沒有「要不要為了它多等」的問題，該開就開。

### 兩件要看的事

**一、最差落在範圍的頭或尾，代表你找到的是邊界，不是最差。**

結果區會標出來。真正的最差可能在你設定的範圍之外，這時候該做的是把範圍拉寬
再掃一次，不是拿這個數字去對規格。

**二、VDDQ 填錯的話，數字看起來完全正常，意思卻不一樣。**

規格的 Vref 中心是 0.5×VDDQ。填錯的話整個掃描窗會平移到眼的邊緣，掃出來的
「最差」量到的是**「離眼中心很遠」**，而不是「Vref 容差內最差」。

工具會拿量到的眼圖準位回頭檢查，窗偏太多就出警告，並**把反推出來的 VDDQ 算給你**。
開發過程中就踩過一次：一條準位中點 0.7577 V 的通道（反推 VDDQ 約 1.515 V），
拿 1.2 V 去算中心，得出「差 28%」的假結論。

**VDDQ 留空也可以跑**，中心會改用量到的準位中點。那樣一定套得上眼，但
**不能拿去對規格**：規格的容差是相對 VDDQ 定義的，用量到的準位當中心等於
拿結果去定義判準，窗永遠不會偏，數字沒有意義。介面會把這件事標出來。

### 峰值不在準位中點

順帶一個實測發現：那條通道的眼寬峰值在 0.8292 V，而準位中點是 0.7577 V。
**眼是不對稱的**，所以以中點為中心的掃描窗，最差幾乎都會落在低壓那一端。

完整數據與介面調查見
[Vref 掃描介面調查](../../validation/ibis/Vref掃描介面調查_20260817.md)。
