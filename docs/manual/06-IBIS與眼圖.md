...
I-V 表超過 100 點時重取樣、補上漏寫的 `[Model Selector]`。

**修的是受管副本，來源檔一個位元都不動**；逐筆可查；**沒有證據就不亂修**。

`[Model Selector]` 那一項是**推測**，所以規則收得很緊：同前綴家族的 `Model_type`
（Input／Output／I/O……）**全部相同才自動建**。不一致就一個位元都不改，改列在
「需要你決定」區——工具沒有把握該綁哪一顆，而綁錯的模型會讓眼圖整個變樣、
卻看起來完全正常。這一項與板子的材料推測都會寫進報告的「警告與限制」段。
...
用 Transient，**八條道只需要求解一次**就拿到全部八張眼圖。
...
而隨機碼型跑 20,000 UI／116.6 秒也只期望出現 1.22 次。
...
一對多接、逐顆粒啟動分析。另附 **CK 邊緣單調性**（AC 區間內回頭＝double clocking
風險，眼圖看不出來）與 **JEDEC slew 極值**（derating 查表用）。
...
- **方向決定選通相位**：寫入時 DQS 已對齊眼中央，讀取是邊緣對齊、要補回 0.5 UI。
...
第一次建立信任前會跳出使用條款：DLL 由 AEDT 在你的機器上實際執行，來源與授權由你負責；
工具做的是掃描與 SHA-256 來源追蹤，**不是安全保證**。全文見
[README 的「執行第三方模型程式庫的條款」](../../README.zh-TW.md#執行第三方模型程式庫的條款)。
...
價值在方向——最壞情況都張得開，正式分析大概率更好；全閉就先調等化。

![秒級預覽：脈衝響應、疊圖與統計眼](../../graph/秒級統計眼_20260829.png)

眼圖下方固定附 **BER 1e-12 外插**（σ 是假設值、原樣標明）。展開「DDR4 Rx 遮罩
統計簽核」填尺寸，下次預覽多一行 **BER 1e-16 的遮罩簽核**。

### 等化掃描：哪一組等化在我的通道上眼最大

把模型的等化選項（PCIe 的 PRESET、CTLE 檔位…）逐組掃過，依統計眼高排名。
PCIe G6 的 P0–P9 約二十秒掃完，點任一列可套進進階 AMI 參數。

實例：USB3.2 Gen2 在 −10 dB 通道上預設檔位全閉，掃 Rx 的 ADC 檔位後
...
**免授權的 S 參數處理**（skrf）：重正規化、換埠序、重取樣、DC 外插、
...
（100GBASE-CR4／KR4／KP4 到 CEI-112G 與 1.2T），不必自己填門檻。**約 8 秒**出結果。

第一行就是結論（`COM = 8.7304 dB（門檻 3 dB） ✅ 通過`），下面逐案例列 VEO、
VEC、Nyquist 插入損耗與封裝長度。**判定取最差的那個案例**——多案例是同一條通道
在不同封裝長度下各算一次。引擎的 HTML 報告與 BATH／FD／TD 圖一併落 `com_reports/`。

![IEEE COM 簽核](../../graph/COM簽核_20260829.png)

> **輸入要求比眼圖分析嚴，不合會在送出前擋下並說明**：必須是**差分 4 埠**
> （2 埠只出頻域曲線），且**最低頻要低於組態的 `F_MIN`**（26 份都是 50 MHz）。
> 從 100 MHz 起的檔案先用工具箱「DC 外插」再送，否則引擎會跑到逾時才停。
...
- 眼圖（每張眼圖卡另出獨立高解析快照）、11 項眼圖量測、DDR 時序裕度表、
  AMI 的等化掃描排名與統計眼。
- 匯出後**拿產出去對請求**：波形時間夠不夠、圖檔空不空、Touchstone 埠數對不對——
  這幾件事出錯時都不會有錯誤訊息。
