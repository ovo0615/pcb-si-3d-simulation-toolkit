...
| 功能 2 | N 段分割 | 沿通道中心脊線尋找切割位置，使切面盡量垂直於走線，輸出 N 個分段 EDB 及 `segments.json`。 |
| 排程模擬 | 依序求解所有分段 | 每段使用獨立的非圖形化 AEDT 工作階段，先執行 Design Validation，完成後匯出 Touchstone。 |
...
![依指定訊號與擴張距離產生裁切範圍](./graph/指定裁切區.png)
...
![裁切完成後的通道 Layout 與 Port](./graph/裁切後結果.png)
...
2. 按「1）分析切割位置」。
3. 工具沿訊號通道建立中心脊線，將切割線配置於等弧長位置，並依局部通道方向旋轉，使切面盡量垂直穿過走線。
4. 檢查每個切面的交點數、最差角度及是否合法。
5. 指定分段輸出資料夾。
6. 所有切面均確認後，按「2）執行 N 段分割」。

![N 段切割位置示意](./graph/N段分割示意.png)

分段過程會：

- 依通道弧長決定各段幾何所有權，避免相鄰分段互相重疊。
- 清除繼承而來但不屬於該段的 Port。
- 在切面訊號端點建立 Gap Port。
...
![第 1 段 Layout](./graph/第1段.png)

![第 2 段 Layout](./graph/第2段.png)

![第 3 段 Layout](./graph/第3段.png)
...
![分段 S 參數自動串接電路示意](./graph/自動串接電路.png)
...
- 「串接電路」：分段及 Port 接線示意。
- 底部系統日誌：顯示 PyEDB、Port、Setup、分段、排程與串接訊息，可複製或清除。
